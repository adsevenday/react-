...
function Home() {
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '1000px', margin: '0 auto' }}>
      <header style={{ justifyContent: 'space-between', marginBottom: '40px' }}>
        ...
        <form onSubmit={handleSearch}>
          <input 
            ...
            style={{ padding: '12px', width: '60%', borderRadius: '5px' }}
          />
        ...
        <div style={{ marginTop: '15px' }}>
          <Link to="/advanced" style={{ color: '#4facfe', textDecoration: 'none' }}>
            🔍 Recherche avancée
          </Link>
        </div>
      </header>
      ...
      {books.length === 0 && !loading && (
        <section>
          ...
          <div style={{ background: '#000000', padding: '15px', borderRadius: '10px' }}>
            {recentChanges.map((change, index) => (
              <div key={index} style={{ borderBottom: '1px solid #e42d2d', padding: '10px 0' }}>
          ...
          return (
            <Link to={`/book/${bookId}`} key={index} style={{ textDecoration: 'none', color: 'inherit' }}>
              <div style={{ border: '1px solid #333', padding: '10px', borderRadius: '8px', textAlign: 'center' }}>
                {book.cover_i ? (
                  <img src={`https://covers.openlibrary.org/b/id/${book.cover_i}-M.jpg`} alt={book.title} style={{ width: '100%' }} />
                ) : (
                  <div style={{ height: '200px', background: '#333' }}>Pas d'image</div>
                )}
